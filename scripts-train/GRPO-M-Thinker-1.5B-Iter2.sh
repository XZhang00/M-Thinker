...
if [ $RANK -eq 0 ]; then
    ...
    MODEL_PATH=XueZhang-bjtu/M-Thinker-1.5B-Iter1

    lr=5e-6
    kl_loss_coef=0.0

    exp_name=M-Thinker-1.5B-RL-Iter2-lr${lr}_kl_loss_coef${kl_loss_coef}-maxlen_24000
    proj_name=DeepSeek-R1-Distill-Qwen-1.5B-GRPO_cold_SFT

    v1_train_path=data/M-Thinker-1.5B-RL-Iter2-data/train.parquet
    v1_test_path=data/M-Thinker-1.5B-RL-Iter2-data/test.parquet

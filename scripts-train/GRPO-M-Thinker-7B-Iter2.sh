...
if [ $RANK -eq 0 ]; then
    ...
    MODEL_PATH=XueZhang-bjtu/M-Thinker-7B-Iter1

    lr=3e-6
    kl_loss_coef=0.0

    exp_name=M-Thinker-7B-RL-Iter2-lr${lr}_kl_loss_coef${kl_loss_coef}-maxlen_24000
    proj_name=DeepSeek-R1-Distill-Qwen-7B-GRPO_cold_SFT

    v1_train_path=data/M-Thinker-7B-RL-Iter2-data/train.parquet
    v1_test_path=data/M-Thinker-7B-RL-Iter2-data/test.parquet
    ...
    python3 -um verl.trainer.main_ppo \
        ...
        data.max_response_length=21952 \
        ...
        custom_reward_function.path=$HOME/verl/verl/utils/reward_score/Reward_acc_lc_cta.py \
